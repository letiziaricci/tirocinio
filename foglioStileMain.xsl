<?xml version="1.0" encoding="UTF-8"?>
    ...
    <xsl:variable name="altroDoc" select="doc('main11.xml')" />
    ...
    <tei:TEI xml:id="{$id}">
          ...
          <tei:teiHeader>
                <tei:fileDesc>
                  ...
                  <xsl:copy-of select="tei:teiCorpus/tei:teiHeader/tei:fileDesc/tei:editionStmt" />
...
<!--    <xsl:template name="stamp" >
    <xsl:choose>
      <xsl:when test="$altroDoc/tei:TEI//tei:div[@type='destination']//tei:stamp">
          <xsl:copy-of select="."/>
      </xsl:when>
      <xsl:when test="$altroDoc/tei:TEI//tei:div[@type='postmark']//tei:stamp">
          <xsl:copy-of select="."/>
      </xsl:when>
      <xsl:otherwise>
         <xsl:copy-of select="$altroDoc/tei:TEI//tei:msIdentifier//tei:idno"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template> -->
